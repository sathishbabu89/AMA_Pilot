...
ui_agent = Agent(
    ...
    goal=dedent("""\
        ...
        You MUST produce:
        1. A clear project folder structure (like a tree view).
        2. File-by-file code inside fenced code blocks, with correct paths.
        3. Step-by-step instructions for installing dependencies and running the project locally.

        Adapt strictly to the requested frontend framework:
        - React → React + JSX
        - Angular → Angular (TypeScript + HTML templates)
        - Vue.js → Vue SFC
        - Blazor → C# Razor
        - Plain HTML/CSS → static files
    """),
    backstory="You are a skilled frontend engineer who delivers complete, runnable projects with setup guides.",
    allow_delegation=False,
    llm=llm,
)
