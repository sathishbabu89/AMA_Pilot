...
def render_agent_output(agent_output):
    """Render agent output with Human-in-the-Loop (HITL) for code review."""
    raw_text = agent_output.raw if agent_output.raw else agent_output.summary
    if not raw_text:
        st.info("⚠️ No output generated.")
        return

    # Enforce consistency (for architect outputs)
    raw_text = enforce_output_consistency(
        agent_output.agent,
        raw_text,
        st.session_state.get("last_user_request", "")
    )
    ...
    st.subheader(f"👨‍💻 Code Review — {agent_output.agent}")
    ...
    for idx, (lang, code) in enumerate(code_blocks, start=1):
        ...
        ext_map = {
            ...
            "python": "py", "csharp": "cs", "java": "java", "dart": "dart",
            "json": "json", "bash": "sh"
        }
        ...
        edited_code = st.text_area(
            ...
            code.strip(),
            height=200,
            key=f"{agent_output.agent}_{file_name}"
        )
        file_map[file_name] = edited_code

    # --- HITL Controls ---
    col1, col2 = st.columns(2)

    with col1:
        if st.button(f"✅ Approve {agent_output.agent} Code"):
            buf = io.BytesIO()
            ...
            st.download_button(
                ...
                data=buf.getvalue(),
                file_name=f"{agent_output.agent}_project.zip",
                mime="application/zip",
            )

    with col2:
        regen_hint = st.text_input(f"💡 Hint for regenerating {agent_output.agent} code")
        if st.button(f"🔄 Regenerate {agent_output.agent} Code"):
            st.session_state[f"{agent_output.agent}_regen_hint"] = regen_hint
            st.warning("⚡ Regeneration not yet wired into agent re-run logic.")
