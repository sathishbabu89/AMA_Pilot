...
from mcp import MCP  # Or just include the class directly
...
mcp_instance = MCP()
test_case_id = 'login_test_case'

# Define a test case (this can be dynamic based on user input)
mcp_instance.create_test_case(
    test_case_id,
    'http://localhost:3000',  # React app URL
    [
        {"type": "fill", "selector": 'input[name="email"]', "value": 'testuser@example.com'},
        {"type": "fill", "selector": 'input[name="password"]', "value": 'password123'},
        {"type": "click", "selector": 'button[type="submit"]'}
    ],
    'http://localhost:3000/dashboard'  # Expected URL after login
)
...
st.set_page_config(
    page_title="🤖 DevHero – Multi-Agent Builder",
    page_icon="🛠️",
    layout="wide"
)
...
# Header
st.markdown(
    "<h1 style='text-align: center; color: #005fae;'>🤖 DevHero – Multi-Agent Workflow Automation</h1>",
...
with col1:
    st.image("devhero.png", caption="DevHero Hero", width=300)
...
if st.session_state.get("show_run_button"):
    ...
    if current_index in st.session_state.phase_results:
        ...
        with col3:
            # Column 3 for Playwright test button
            if st.button("Run Playwright Test for Login", use_container_width=True):
                with st.spinner("Running Playwright Test..."):
                    try:
                        #run_playwright_test()  # Run the async test in an event loop
                        test_case_id = "your_test_case_id"  # or generate one dynamically if needed
                        result = mcp_instance.run_test(test_case_id)
                        st.write(result)
                    except Exception as e:
                        st.error(f"❌ Playwright test failed: {e}")
